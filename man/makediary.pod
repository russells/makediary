...
B<makediary> can generate diaries in several different paper sizes.
B<makediary> can include events printed on dates specified in a F<.calendar>
file.  The F<.calendar> file is in almost the same format as used by pcal(1),
...
When printing images, output colour.  By default, images are converted to grey
scale.  Note that this does not apply to included EPS files, which will always
be in whatever colours they come with.

=item --colour-images

This is an alias for B<--colour>.  It's more descriptive, as this option only
applies to images.
...
=item --cover-page-image=I<imagefile>

Print I<imagefile> on the cover of the diary, replacing the other markup on the
cover.  The image will be maximised to fit on the page.  Because all the text
is replaced, including the year text, it may be a good idea to put the year in
the image somewhere.

=item --day-to-page

This option is deprecated, and it is equivalent to B<--layout=day-to-page>.
...
=item --eps-page=I<epsfilename>[|I<title>]

Include a page whose content is I<epsfilename>.  A title can be put on the page
by appending it after the file name, separated by a vertical bar.  This could
be useful for including maps, for instance.

You will probably have to quote the vertical bar to prevent interpretation by
the shell.

=item --eps-2page=I<epsfilename>[|I<lefttitle>[|I<righttitle>]]

Include two pages whose content is I<epsfilename>, which will be split across a
pair of facing left and right pages.  If specified, the pages will have titles
I<lefttitle> and I<righttitle>.

You will probably have to quote the vertical bars to prevent interpretation by
the shell.

=item --ref=I<name[|title[|title]]>

Include a page or pages whose content is I<name>, possible with page titles.
This could be useful for including maps, for instance.

If I<name> contains any '/' characters, it will be interpreted as a path to a
...
files is expanded as C<eps/>I<name>C</>I<name>C<.eps> or
C<eps/>I<name>C</>I<name>C<.*.eps>.  These strings will be appended to
each element of C<sys.path> until one of them expands to one or more file
names.  These file names will then be used as the EPS files.

If one I<title> is supplied, it will become the page title for each of the
included pages.  To specify different titles for each page, separate them with
'|'.  To put a title on only the first page, use C<--ref=name'|title|'>, so
that subsequent titles are empty.  If there are more pages than titles, the
extra pages will have no titles.  To put a line at the top of a page without a
title, specify that title as a single space.
...
=item --layout=day-to-page

=item --layout=week-to-opening

=item --layout=week-to-2-openings
...
instead of from the Southern Hemisphere.  Implies B<--moon>.
...
Output to I<filename>.  Defaults to F<diary.ps> if generating PostScript, or
F<diary.pdf> if generating PDF.  A lone 'B<->' means standard output.
...
None of the extensions mentioned in "PCAL(1) F<.calendar> FILE" will work, but
...
=item --perpetual-calendars

Print four pages of perpetual calendars.  There are only 14 possible calendars,
and this options makes B<makediary> print them all, along with a list of recent
...
Print a sed reference page.  (Equivalent to C<--ref=sed'|sed reference'>).

=item --sh-ref

Print the Unix shell reference page.  (Equivalent to C<--ref=sh'|Shell and
utility reference'>).
...
Print the Units reference page.  (Equivalent to C<--ref=units'|Units'>.)

When installing B<makediary>, this option requires that the GNU units program
be available.  Because some systems have that program as gunits, and because
there are other possibilities about where units could be installed,
B<makediary> will first look for the program named by $UNITS, and if that is
not found look for gunits, and if that is not found, finally try units.

Similarly, awk is required for installation.  B<makediary> will try $AWK, gawk,
then awk.

=item --unix-ref

Print the Unix commands reference page.  (Equivalent to C<--ref=unix'|Unix
reference'>.)
...
Print the vi and vim reference pages.  The C<--vi-ref> and C<--vim-ref> options
do exactly the same thing.  (Equivalent to C<--ref=vi'|Vi reference|Vim
extensions'>.)

=item --week-to-opening

This option is deprecated, and is equivalent to B<--layout=week-to-opening>.
...
This section is not relevant if you use the B<--pcal> or B<--pcal-planner>
flags.  If you use pcal to parse the F<.calendar> file, see pcal(1).
...
pcal(1), with some extensions, and some unimplemented features.
...
These things from the pcal() F<.calendar> format are not recognised:

=over

=item the "opt" keyword,

=item the "every" keyword,

=item the "each" keyword,

=item the "all" keyword.
...
B<makediary> will read several files containing configuartion information for
the Personal Information page.  The files are read in the order:

=over

=item F<.makediaryrc> in the user's home directory

=item F<.makediaryrc> in the current directory
...
This is an example F<.makediaryrc> file:
...
the tenth calendar year after publishing (ie the start of 2019 for version 0.2.96).
